...
// --- the key binds task, repo, head, kind and occurrence exactly ------------
...
assert.equal(keys.size, 6, 'every binding component changes the idempotency key');
assert.match(terminal.idempotencyKey, /^sha256:[0-9a-f]{64}$/);

// --- a regression receipt is a typed failure, not a completion --------------
const regression = buildExecutionReceipt({
