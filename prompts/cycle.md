...
- `verified` — merged and waiting on a deployment. `smoke` moves it to
  `released` on its own once production actually contains it and the smoke
  checks pass. You do not deploy, and you do not move it by hand. If it dropped
  back to `reported_done`, the PR head moved and the old green run no longer
  applies; wait for checks on the new commit.
